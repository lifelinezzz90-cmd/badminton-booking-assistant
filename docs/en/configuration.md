...
Add `courtPriority` only when changing the built-in order. A failed court does not stop later candidates. Add `vpn.shortcutPath` only if system and user Start Menu discovery fails.
